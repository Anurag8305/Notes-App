...
import NotesCard from './Toodos/NotesCard';
...
const UserNotes = () => {
  ...
  const getNotes=()=>{
    if(token){
        axios.get("https://long-gray-tam.cyclic.app/notes/notes",{
      headers: {
  ...
  return (
    ...
    {loader?(<img src="https://cdn.dribbble.com/users/1787505/screenshots/7300251/media/a351d9e0236c03a539181b95faced9e0.gif"/>):notes?.map((el)=>{
        return <UserNotesCard key={el._id} category={el.category} notes={el.note} id={el._id} titles={el.title} getNotes={getNotes} />
      })}
    {/* {notes?.map((el)=>{
